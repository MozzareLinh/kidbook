...
//Lấy thời gian còn lại từ Firestore
  ...
  List<Story> _getRandomVideos(List<Story> videos, int count) {
    final random = Random();
  ...
  //Kiểm tra nếu story đã có trong danh sách yêu thích
    ...
    try {
      ...
      if (userId != null) {
        ...
        if (userSnapshot.exists) {
          ...
          // Đặt lại trạng thái
          setState(() {
            // Kiểm tra xem storyId hiện tại có nằm trong danh sách story trong favorites không
            isFavorite = favorites['story'] != null &&
                (favorites['story'] as List<dynamic>).contains(widget.storyId);
          });
        } else {
          print('Không tìm thấy người dùng với userId: $userId');
        }
  ...
  // Hàm thêm hoặc bỏ story khỏi danh sách yêu thích
  Future<void> _toggleFavorite() async {
    ...
    if (userSnapshot.exists) {
      ...
      try {
        // Lấy danh sách favorites (bao gồm story, gol, comic, ...)
        Map<String, dynamic> favorites =
            Map<String, dynamic>.from(userSnapshot['favorites'] ?? {});

        // Lấy danh sách story hiện tại trong favorites
        List<dynamic> storyFavorites =
            List<dynamic>.from(favorites['story'] ?? []);

        // Kiểm tra nếu storyId đã có trong danh sách yêu thích hay chưa
        if (storyFavorites.contains(widget.storyId)) {
          // Nếu có, thì xóa khỏi danh sách
          storyFavorites.remove(widget.storyId);
        } else {
          // Nếu chưa, thì thêm vào danh sách
          storyFavorites.add(widget.storyId);
        }

        // Cập nhật lại danh sách story trong favorites
        favorites['story'] = storyFavorites;
        ...
        builder: (context, orientation) {
          ...
          return Column(
            children: [
                  ...
                  child: _controller.value.isInitialized
                      ? GestureDetector(
                          ...
                          child: Stack(
                            ...
                            children: [
                              AspectRatio( //điều chỉnh kích thước khung hình theo tỉ lệ đã định nghĩa
                                            ...
                                            data: SliderTheme.of(context)
                                              ...
                                              overlayShape:
                                                  const RoundSliderOverlayShape(
                                                      overlayRadius:
                                                          16.0), //Hiệu ứng khi kéo nút tròn
                                              thumbColor: Colors
                                                  .deepPurple, //Màu của nút tròn
                                            ),
                                            child: Slider(
                                              min: 0.0,
                                              max: value.duration.inSeconds
                                                  .toDouble(), //tổng thời gian của video
                                              value: value.position.inSeconds
                                                  .toDouble(),  //vị trí hiện tại
                                              onChanged: (newValue) {
                                                setState(() {
              ...
              if (!isLandscape) const SizedBox(height: 5.0),
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          // Tạo khoảng cách giữa các video
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      _formatDuration(video
                                          .duration), // Hiển thị thời gian video
                                      style: const TextStyle(
                                        color: Colors.black,
                                        fontSize: 12.0,
                                      ),
